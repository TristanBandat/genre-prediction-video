...
def main():
    ...
    train_ds = tfds.load('music4_all_onion_dc:3.1.0', data_dir='data/', batch_size=64,
                         as_supervised=True, split='train')
    test_ds = tfds.load('music4_all_onion_dc:3.1.0', data_dir='data/', batch_size=64,
                        as_supervised=True, split='test')
    valid_ds = tfds.load('music4_all_onion_dc:3.1.0', data_dir='data/', batch_size=64,
    ...
    log_dir = os.path.join('logs', 'Deep_NN', 'VGG19', 'fit', datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
