...
def main():
    ...
    train_ds = tfds.load('music4_all_onion_dc:1.1.0', data_dir='data/', batch_size=256,
                         as_supervised=True, split='train')
    test_ds = tfds.load('music4_all_onion_dc:1.1.0', data_dir='data/', batch_size=256,
                        as_supervised=True, split='test')
    valid_ds = tfds.load('music4_all_onion_dc:1.1.0', data_dir='data/', batch_size=256,
    ...
    log_dir = os.path.join('logs', 'Simple_NN', 'INCP', 'fit', datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
